import {
  Box, Typography, Chip, Divider, TextField,
...
export default function OdontogramPanel({ readOnly }) {
  const {
    actions, selectedAction, selectAction,
    selectedTooth, data,
    toggleAbsent, clearTooth, setToothNote,
  } = useOdontogramStore()
  ...
  const FACE_LABELS = {
    top: 'Oclusal', bottom: 'Lingual',
    left: 'Mesial', right: 'Distal', center: 'Vestibular',
  }
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
          {actions.map(a => (
            <Chip
              ...
              label={a.name}
              size="small"
              onClick={() => !readOnly && selectAction(selectedAction === a.name ? null : a.name)}
              sx={{
        ...
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
            <Typography variant="subtitle2" fontWeight={500}>
              Diente {tooth.number}
              {tooth.absent && (
                <Chip label="Ausente" size="small" color="error" sx={{ ml: 1 }} />
              )}
            </Typography>
            {!readOnly && (
              <Box>
                <Tooltip title={tooth.absent ? 'Marcar presente' : 'Marcar ausente'}>
                  <IconButton size="small" onClick={() => toggleAbsent(tooth.number)}>
                    <BlockIcon fontSize="small" color={tooth.absent ? 'error' : 'inherit'} />
                  </IconButton>
                </Tooltip>
                <Tooltip title="Limpiar diente">
                  <IconButton size="small" onClick={() => clearTooth(tooth.number)}>
