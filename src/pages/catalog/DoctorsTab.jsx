...
import {
  ...
  Typography, Chip, Card, CardContent,
} from '@mui/material'
      ...
      <CardContent sx={{ pb: '12px !important' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Typography variant="body2" fontWeight={500} noWrap>{d.profile?.full_name ?? '—'}</Typography>
            <Typography variant="caption" color="text.secondary" noWrap>{d.profile?.email}</Typography>
          </Box>
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            <IconButton size="small" onClick={() => onEdit(d)}><EditIcon fontSize="small" /></IconButton>
            <IconButton size="small" onClick={() => onDelete(d.id)}><DeleteIcon fontSize="small" color="error" /></IconButton>
          </Box>
        </Box>
        <Box sx={{ display: 'flex', gap: 1, mt: 1, flexWrap: 'wrap', alignItems: 'center' }}>
          {d.specialty ? (
            <Chip label={d.specialty.name} size="small"
              sx={{ bgcolor: d.specialty.color + '22', color: d.specialty.color, borderColor: d.specialty.color }}
              variant="outlined" />
          ) : (
            <Typography variant="caption" color="text.secondary">Sin especialidad</Typography>
          )}
          {d.license && (
            <Typography variant="caption" color="text.secondary">N.° {d.license}</Typography>
          )}
        </Box>
      </CardContent>
    </Card>
...
export default function DoctorsTab({ onNotify }) {
  ...
  const handleSave = async () => {
    if (!form.profile_id) { onNotify('Selecciona un perfil.', 'error'); return }
    const fn = editId
  ...
  const handleDelete = async id => {
    ...
    if (error) onNotify(error, 'error'); else onNotify('Doctor desactivado.')
  }
  ...
  return (
    <Box>
      ...
      {isMobile ? (
        <Box>
          {doctors.length === 0
            ? <Typography color="text.secondary" textAlign="center" mt={4}>No hay doctores registrados</Typography>
            : doctors.map(d => <DoctorCard key={d.id} d={d} onEdit={openEdit} onDelete={handleDelete} />)
          }
        </Box>
      ) : (
        /* Vista desktop */
        <TableContainer component={Paper} variant="outlined">
          <Table size="small">
            ...
            <TableBody>
              {doctors.length === 0 && (
                <TableRow><TableCell colSpan={4} align="center" sx={{ py: 4, color: 'text.secondary' }}>No hay doctores registrados</TableCell></TableRow>
              )}
              {doctors.map(d => (
                <TableRow key={d.id} hover>
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      ...
                      <Box>
                        <Typography variant="body2" fontWeight={500}>{d.profile?.full_name ?? '—'}</Typography>
                        <Typography variant="caption" color="text.secondary">{d.profile?.email}</Typography>
                      </Box>
                    </Box>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell><Typography variant="body2">{d.license ?? '—'}</Typography></TableCell>
                  <TableCell align="right">
                    <Tooltip title="Editar"><IconButton size="small" onClick={() => openEdit(d)}><EditIcon fontSize="small" /></IconButton></Tooltip>
                    <Tooltip title="Desactivar"><IconButton size="small" onClick={() => handleDelete(d.id)}><DeleteIcon fontSize="small" color="error" /></IconButton></Tooltip>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="xs" fullWidth fullScreen={isMobile}>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important' }}>
          {!editId && (
            <TextField select label="Perfil (rol DOCTOR)" value={form.profile_id} onChange={set('profile_id')} size="small" fullWidth>
              {doctorProfiles.length === 0 && <MenuItem disabled>No hay perfiles DOCTOR disponibles</MenuItem>}
              {doctorProfiles.map(u => <MenuItem key={u.id} value={u.id}>{u.full_name} — {u.email}</MenuItem>)}
            </TextField>
          )}
