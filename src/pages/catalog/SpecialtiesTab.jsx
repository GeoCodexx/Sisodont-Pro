...
import {
  ...
  CircularProgress, Chip, Typography, Alert,
} from "@mui/material";
...
// ─────────────────────────────────────────────────────────────
export default function SpecialtiesTab({ onNotify, isSuperAdmin }) {
  const { specialties, saving, createSpecialty, updateSpecialty, deleteSpecialty } =
    useCatalogStore();
  ...
  const handleSave = async () => {
    if (!form.name.trim()) { onNotify("El nombre es requerido.", "error"); return; }
    ...
    <Box>
      ...
      {!isSuperAdmin && (
        <Alert
          ...
          sx={{ mb: 2 }}
        >
          La lista de especialidades son administrados globalmente por el area de soporte.
        ...
        <Table size="small">
          ...
          <TableBody>
            {specialties.length === 0 && (
              <TableRow>
                <TableCell colSpan={isSuperAdmin ? 3 : 2} align="center"
